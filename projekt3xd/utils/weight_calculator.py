...
class WeightCalculator:
    ...
    def get_weights_from_user(self) -> Dict[str, float]:
        ...
        for i, criterion in enumerate(criteria):
            # Dla ostatniego kryterium automatycznie przypisz pozostałą wagę
            if i == len(criteria) - 1:
                self.weights[criterion] = round(remaining, 2)
                print(f"{criterion.capitalize()}: {remaining:.2f} (automatycznie przypisana pozostała waga)")
                break
                
            while True:
                try:
                    print(f"\nPozostało do rozdzielenia: {remaining:.2f}")
                    weight = input(f"{criterion.capitalize()} (wciśnij ENTER aby przypisać 0): ")
                    
                    if not weight:  # Jeśli użytkownik wcisnął ENTER
                        self.weights[criterion] = 0.0
                        print(f"{criterion.capitalize()}: 0.00")
                        break
                    ...
                    self.weights[criterion] = round(weight, 2)  # Zaokrąglamy do 2 miejsc po przecinku
